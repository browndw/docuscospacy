...
Like **frequency_table**, functions requires a tuple of the type generated by the **convert_corpus** function. It also requires an integer representing the size of the ngrams to be returned, as well as the total number of tokens against which to normalize (per million tokens).
